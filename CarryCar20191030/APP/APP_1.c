...
void APP_Init(void)
{
	...
	if(Openmv.line_parl<4&&task_flag==2)
	{
		if(segment_flag==0&&task_flag==2&&delta_cnt<650)
		{
	  ...
	  else if(segment_flag==0&&task_flag==2&&delta_cnt>650)
	  {
		  	Set_GoForward_ALL();
	...
	else  if(Openmv.line_parl>=4)
	{
		...
		if(task_flag==3&&delta_cnt<1500&&time_flag==1)
		{
			  PID_General(90,Openmv.theta_vertical,&pid_line_theta);
				Set_GoForward_ALL();
				Motor_Control( pid_line_x.output-pid_line_theta.output + 400,\
		...
		else if(task_flag==3&&delta_cnt>1500)
		{
			 	static u16 n3 = 0;
				Set_Break_All();
				Motor_Control(0,0,0,0);
		...
		if(Openmv.line_parl<2&&task_flag==4)
	 {
				if(segment_flag==0&&task_flag==2&&delta_cnt<650)
				{
						PID_General(90,Openmv.theta_vertical,&pid_line_theta);
	  ...
	  else if(segment_flag==0&&task_flag==4&&delta_cnt>650)
	  {
		  	Set_GoForward_ALL();
		    Motor_Control(300,200,200,200);
	  }
